...
# Edit 1: Top UI Elements
edit1 = '''// DOM Elements for UI
...
diffBtns.forEach(btn => {
    btn.addEventListener('click', (e) => {
        ...
        switch(level) {
            ...
            case 'grandmaster': botDepth = 4; break; // Depth 4 is good max for raw JS
        }
        gameMode = 'bot';
        diffModal.classList.add('hidden');
        restartGame();
...
# Edit 2: Camera Controls zoom
target2 = '''    controls.maxPolarAngle = Math.PI / 2.2;
    controls.minDistance = 10;
    controls.maxDistance = 40;'''
edit2 = '''    controls.maxPolarAngle = Math.PI / 2.2;
    controls.minDistance = 10;
    controls.maxDistance = 60; // Increased to see people'''
text = text.replace(target2, edit2)

# Edit 3: Legs and chairs
target3 = '''    // Wooden Table
...
# Edit 4: Move flow modifications
target4 = '''    actColor = actColor === 'w' ? 'b' : 'w';
    updateDraw3D();
    updateTurnIndicator();
    checkGameEnd();
...
# Edit 6: Env Restart setup
target6 = '''function restartGame() {
    // Clear 3D meshes
...
# Edit 7: Replace old Bot Engine from line 771 to the end
bot_target_start = text.find('// AI Minimax Bot (Retained Logic)')
if bot_target_start != -1:
    text = text[:bot_target_start]
...
function makeBotMove() {
    ...
    // Depth bounds:
    // If bot Depth is 4, but we don't want JS to hang for 3 seconds on every move:
    // Limit width if it takes longer, but simple alpha beta up to depth 4 is usually okay 
    // in mid-late game. In opening, we might limit to depth 3 to stay responsive.
    let piecesCount = 0;
    board.forEach(row => row.forEach(col => { if(col) piecesCount++; }));
    if (botDepth === 4 && piecesCount > 24) evaluateDepth = 3; // Keep memory from hanging
    ...
    for (let mObj of allMoves) {
        ...
        let noise = 0;
        if(botDifficulty === 'novice') noise = (Math.random() * 40 - 20); // Massive noise 
        if(botDifficulty === 'intermediate') noise = (Math.random() * 20 - 10);
        if(botDifficulty === 'advanced') noise = (Math.random() * 6 - 3);
        if(botDifficulty === 'professional') noise = (Math.random() * 2 - 1);
        if(botDifficulty === 'grandmaster') noise = (Math.random() * 0.1 - 0.05);
